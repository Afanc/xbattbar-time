...
char time_remaining_buffer[9];  /* battery time remaining */
...
void showdiagbox(void)
{
  ...
  sprintf(diagmsg,
         //"AC %s-line: battery level is %d%%. %s left",
	 "%d%% %s",
         //ac_line ? "on" : "off", battery_level, time_remaining_buffer);
	 battery_level, time_remaining_buffer);
...
#define TIME_STRING 		"remaining="
void print_script_error(void)
{
	fprintf(stderr, "\nExternal script must print three strings:\n"
...
void battery_check(void)
{
	...
	if (pid) { /* parent */
		...
		time = strstr(buffer, TIME_STRING);
		if (!time) {
			print_script_error();
			goto exit_check;
		}
